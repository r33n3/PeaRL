...
@pytest.mark.asyncio
async def test_full_lifecycle_contract(reviewer_client):
    ...
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/org-baseline", json=baseline)
    assert r.status_code == 200
    assert r.json()["kind"] == "PearlOrgBaseline"

    # 3. Attach app spec
    spec = load_example("project/app-spec.request.json")
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/app-spec", json=spec)
    assert r.status_code == 200
    ...
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/environment-profile", json=profile)
    assert r.status_code == 200

    # 5. Compile context
    compile_req = load_example("compile/compile-context.request.json")
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/compile-context", json=compile_req)
    ...
    r = await reviewer_client.get(f"/api/v1/projects/{pid}/compiled-package")
    assert r.status_code == 200
    pkg = r.json()
    ...
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/task-packets", json=tp_req)
    assert r.status_code == 201
    tp = r.json()
    ...
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/remediation-specs/generate", json=rem_req)
    assert r.status_code == 201
    rem = r.json()
    ...
    r = await reviewer_client.post(f"/api/v1/projects/{pid}/reports/generate", json=report_req)
    assert r.status_code == 200
    report = r.json()
