...
@pytest.mark.asyncio
async def test_outcome_execution_phase_fallback(db_session):
    """Task packet with outcome={} and execution_phase='complete' produces outcome='achieved'."""
    pid = await _create_project(db_session)
    frun_id = generate_id("frun")
    tp_id = await _create_task_packet(db_session, pid, outcome={}, execution_phase="complete")
